function detector = acfTrain( varargin )%varargin是可变参数列表，variable argument in的意思
...
% accuracy on rigid（严格的） object detection. Please see acfReadme.m for details.
%
...
opts = initializeOpts( varargin{:} );%varargin是可变参数列表
...
for stage = 0:numel(opts.nWeak)-1
  ...
  % compute local decorrelation filters % 计算局部去相关滤波器
  ...
  % compute features for positives %计算正样本特征
  if( stage==0 )
  ...
  % sample negatives and compute features %负样本与特征计算
  Is0 = sampleWins( detector, stage, 0 );
  X0 = chnsCompute1( Is0, opts ); clear Is0;
  X0 = reshape(X0,[],size(X0,4))';
  
  % accumulate negatives from previous stages %累积前几个阶段的负面影响
  if( stage>0 )
...
function opts = initializeOpts( varargin )  %初始化参数%varargin是可变参数列表variable argument in的意思
% Initialize opts struct.
...
opts = getPrmDflt(varargin,dfs,1);%varargin是可变参数列表variable argument in的意思
% fill in remaining parameters 填入剩下的参数
...
% initialize pNms, pBoost, pBoost.pTree, and pLoad 初始化部分参数
...
% Load or sample windows for training detector. %加载窗口或采样（动词）窗口用于训练检测器。
...
if( exist(crDir,'dir') && stage==0 )
  % if window directory is specified simply load windows如果指定了窗口目录，只需加载窗口
  ...
  % sample windows from full images using sampleWins1()使用sampleWins1（）从完整图像中采样窗口
...
% optionally jitter positive windows可选择抖动正窗口
...
%确保dims可以通过收缩整除，并且不小于modelDsPad
...
% optionally save windows to disk and update log 选择性保存并更新日志
...
% Sample windows from I given its ground truth gt.正确打标记的数据成为ground
% truth，即gt
...
if( positive ), bbs=gt; bbs=bbs(bbs(:,5)==0,:); else
  if( stage==0 )
    % generate candidate bounding boxes in a grid在网格中生成候选边界框
    ...
    % run detector to generate candidate bounding boxes运行检测器以生成候选边界框
    bbs=acfDetect(I,detector); [~,ord]=sort(bbs(:,5),'descend');
    bbs=bbs(ord(1:min(end,opts.nPerNeg)),1:4);
  end
  if( ~isempty(gt) )
    % discard any candidate negative bb that matches the gt丢弃任何与gt匹配的候选负bb
    n=size(bbs,1); keep=false(1,n);
    for i=1:n, keep(i)=all(bbGt('compOas',bbs(i,:),gt,gt(:,5))<.1); end
    bbs=bbs(keep,:);
  end
end
% grow bbs to a large padded size and finally crop windows将bbs增长到一个大的填充尺寸，最后裁剪窗户
...
% Compute single scale channels of dimensions modelDsPad.%计算单尺度通道
...
parfor i=1:n   %parfor中的i不能保证是按照顺序迭代的
...
% Compute filters capturing local correlations for each channel.计算过滤器捕获每个通道的本地相关性。
fprintf('Computing correlations... '); start=clock;
[~,~,m,n]=size(chns); w=wFilter; wp=w*2-1;
filters=zeros(w,w,m,nFilter,'single');
for i=1:m
  % compute local auto-scorrelation using Wiener-Khinchin theorem使用Wiener-Khinchin定理计算局部自相关
  ...
  % compute filters for each channel from sig (sorted by eigenvalue)从sig计算每个通道的过滤器（按特征值排序）

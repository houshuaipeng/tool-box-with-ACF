...
% of images (or filenames). In the first case, the return is a set of bbs（bounding box边界框）
...
% bbs are saved to a comma（逗号） separated text file and the output is set to
...
%
% A cell of detectors trained with the same channels can be specified,（可以指定使用相同通道训练的探测器单元）
% detected bbs from each detector are concatenated.（从每个探测器检测到的bbs被连接起来） If using multiple
...
if(nargin<3), fileName=''; end; multiple=iscell(I);%若指定第三个参数，则令filename为空。
                                                   %若I为cell array，则multiple为1，否则为0.
...
d=fileparts(fileName); if(~isempty(d)&&~exist(d,'dir')), mkdir(d); end %mkdir用于新建文件夹
...
imreadf=opts.imreadf; imreadp=opts.imreadp;%imreadf将调用函数imread
shrink=pPyramid.pChns.shrink; pad=pPyramid.pad;
separate=nDs>1 && isfield(pNms,'separate') && pNms.separate;
%isfield检测结构体pNms是否包含有separate指定的域，如果包含，返回逻辑1
% read image and compute features (including optionally applying filters)
if(all(ischar(I))), I=feval(imreadf,I,imreadp{:}); end %ischar用于判断是否为字符，若是则返回1；all用于判
                                                       %断矩阵是否每个元素都为非零，若是，返回1
                                                       %feval函数表示调用imread的句柄imreadf，其参数为I,imreadp{:}
P=chnsPyramid(I,pPyramid); bbs=cell(P.nScales,nDs);
if(isfield(opts,'filters') && ~isempty(opts.filters)), shrink=shrink*2;
    %isfield检测结构体opts是否包含有filters指定的域，如果包含，返回逻辑1
